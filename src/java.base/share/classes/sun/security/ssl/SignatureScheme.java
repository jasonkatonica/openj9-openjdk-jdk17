...
enum SignatureScheme {
    ...
    static List<SignatureScheme> getSupportedAlgorithms(
        ...
        // when explicitly set via System property.
        List<SignatureScheme> schemesToCheck = null;
        if (config.signatureSchemes.isEmpty()) {
            List<SignatureScheme> schemes = Arrays.asList(SignatureScheme.values());
            schemesToCheck = new ArrayList<>(schemes.size());
            for (SignatureScheme scheme: schemes) {
                if (scheme != ECDSA_BRAINPOOLP512R1TLS13_SHA512) {
                    ...
                    if (SSLLogger.isOn &&
                        ...
                        SSLLogger.finest("Ignore " + ECDSA_BRAINPOOLP512R1TLS13_SHA512.name + " from supported signature schemes");
                    }
                }
            }
        } else {
