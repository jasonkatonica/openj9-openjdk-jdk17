...
public final class RestrictedSecurity {
    ...
    private static final class RestrictedSecurityProperties {
        ...
        private void update(String profileExtensionId) {
            try {
                // Load restricted security providers from java.security properties.
                List<Integer> updatedProvidersPosition = updateProviders(profileExtensionId);
                // Load restricted security properties from java.security properties.
        ...
        private void initProviders(String profileID) {
            ...
            for (int pNum = 1;; ++pNum) {
                ...
                if (providerInfo.trim().isEmpty()) {
                    printStackTraceAndExit(
                        "Cannot specify an empty provider for " + property);
                }
        ...
        private List<Integer> updateProviders(String profileExtensionId) {
            List<Integer> modifiedProvidersPosition = new ArrayList<>();
            ...
            for (int i = 1; i <= numOfExistingProviders; i++) {
                ...
                if (providerInfo != null) {
                    if (!providerInfo.trim().isEmpty()) {
                        ...
                        modifiedProvidersPosition.add(i);
                    } else {
                        // Trigger action to remove provider.
            ...
            return modifiedProvidersPosition;
        }
        ...
        private void setConstraints(String providerName, String providerInfo) {
            if (debug != null) {
...
"\\[([\\+\\-]?)(\\{\\w+,[A-Za-z0-9\\.]+,[A-Za-z0-9\\=\\*:]+\\})(,\\{\\w+,[A-Za-z0-9\\.]+,[A-Za-z0-9\\=\\*:]+\\})*\\]");
         ...
         */
        private void initConstraints(String profileID) {
            if (debug != null) {
            ...
            for (int pNum = 1; pNum <= providersSimpleName.size(); pNum++) {
                ...
                setConstraints(providerName, providerInfo);
            }
        ...
        private void updateConstraints(String profileExtensionId, List<Integer> updatedProvidersPosition) {
            for (int position: updatedProvidersPosition) {
                String providerName = providersSimpleName.get(position - 1);
                String providerInfo = securityProps
                        .getProperty(profileExtensionId + ".jce.provider." + position);
